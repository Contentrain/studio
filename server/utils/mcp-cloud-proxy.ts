...
import { mcpReadRetryAfterSeconds, mcpWriteRetryAfterSeconds } from '~~/server/utils/mcp-github-budget'
...
export async function runMcpCloudProxy(
  ...
  // Read tools stop only at a much lower floor (`readReserveFor`).
  if (toolCalls.length > 0) {
    const isWrite = toolCalls.some(name => WRITE_TOOL_NAMES.has(name))
    const waitSeconds = isWrite
      ? mcpWriteRetryAfterSeconds(ctx.installationId)
      : mcpReadRetryAfterSeconds(ctx.installationId)
    if (waitSeconds !== null) {
