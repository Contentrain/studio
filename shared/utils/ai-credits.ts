 ...
 *
 * 60 credits = $1.80 of tracked spend. A turn costing more than that
 * is still metered (and billed as overage) only up to this cap — the
 * gap between real spend and metered credits above the cap is an
 * accepted, bounded loss on that single message, not a full write-off
 * (raised from 30 → 60 in 2026-09 after measurement showed some heavy
 * editorial turns landing past the old $0.90 cap). A true per-message
 * hard cutoff isn't possible here: Anthropic bills for tokens already
 * generated by the time a turn's total cost is known, so nothing can
 * block spend retroactively — see `SS-14-unit-economics.md` for why a
 * proactive mid-turn abort (stopping the tool loop early once running
 * cost crosses a ceiling) was scoped out as a separate, larger change.
 */
export const MAX_CREDITS_PER_MESSAGE = 60
